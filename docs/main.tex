...
\begin{enumerate}
    ...
    \item \textbf{Case-aware state representation.} 
    Unlike prior work where the state captures only the global process snapshot for resource dispatching, our state includes case-specific features---such as the case's current position, SLA urgency, and branching probabilities---enabling the agent to reason about both routing and resource allocation jointly.
...
The state $s \in \mathcal{S}$ provides the agent with a numerical snapshot of the simulation at each decision point. Unlike \cite{Learning_policies_drl, Optimizing_Resource_Allocation_Policies}, where the state captures only global process information for resource dispatching, our formulation additionally includes features of the specific case $c$ for which the agent selects both the next activity and its executing resource. All features are normalized to $[0, 1]$ to improve sample efficiency \cite{sutton_2018_rl_intro}.

Let $R = \{r_1, \dots, r_{|R|}\}$ be the ordered set of resources, $A = \{a_1, \dots, a_{|A|}\}$ the ordered set of activities, $\sigma_c = \langle e_1, \dots, e_n \rangle$ the prefix of case $c$, and $T$ the SLA threshold. The state is defined by the following components:

\begin{itemize}

    \item \textbf{Utilization} ($u_i$):
    The fraction of resource $r_i$'s capacity currently in use:
    \[
        u_i = \frac{\text{count}(r_i)}{\text{cap}(r_i)}.
    \]

    \item \textbf{Assignment} ($\eta_i$):
    An encoding of the activity that resource $r_i$ is currently executing:
    \[
        \eta_i =
        \begin{cases}
    ...
    \item \textbf{Queue Pressure} ($q_i$):
    The number of cases waiting for resource $r_i$, normalized and clamped:
    \[
        q_i = \min\!\left(\frac{|\text{queue}(r_i)|}{\theta_q},\; 1\right).
    \]

    \item \textbf{Pending Demand} ($\kappa_j$):
    The number of cases awaiting execution of activity $a_j$:
    \[
        \kappa_j = \min\!\left(\frac{|\text{pending}(a_j)|}{\theta_q},\; 1\right).
    \]

    \item \textbf{Last Activity} ($\ell_c$):
    The position of case $c$ in the process:
    \[
        \ell_c =
        \begin{cases}
            j / |A|, & \text{if the last executed activity is } a_j, \\
            0, & \text{if } \sigma_c = \langle\rangle.
        \end{cases}
    \]

    \item \textbf{Trace Length} ($\lambda_c$):
    The number of activities executed so far in case $c$:
    \[
        \lambda_c = \min\!\left(\frac{|\sigma_c|}{\theta_\lambda},\; 1\right).
    \]

    \item \textbf{SLA Urgency} ($\phi_c$):
    The elapsed time of case $c$ relative to the SLA threshold:
    \[
        \phi_c = \min\!\left(\frac{t_{\text{now}} - t_s(e_1)}{T},\; 1\right).
    \]

    \item \textbf{Branching Probabilities} ($\mathbf{b}_c$):
    The transition probability vector from the routing policy at the current position of case $c$:
    \[
        \mathbf{b}_c = \big[\, P(a_j \mid a_{\text{last}}) \,\big]_{j=1}^{|A|}.
    \]

    \item \textbf{Hour of Day} ($\tau_h$):
    The normalized time within the current day:
    \[
        \tau_h = \frac{t_{\text{now}} \bmod 86400}{86400}.
    \]

    \item \textbf{Day of Week} ($\tau_d$):
    The normalized day within the current week:
    \[
        \tau_d = \frac{\lfloor t_{\text{now}} / 86400 \rfloor \bmod 7}{6}.
    \]

\end{itemize}

The temporal features allow the agent to learn calendar-dependent patterns implicitly \cite{Optimizing_Resource_Allocation_Policies}. 
The full state vector $s \in \mathbf{R}^{d}$ concatenates all components:
\begin{align*}
    s = \big[\, &\underbrace{u_1, \eta_1, q_1, \;\dots,\; u_{|R|}, \eta_{|R|}, q_{|R|}}_{\text{global: } 3|R|}, \\
    &\underbrace{\kappa_1, \dots, \kappa_{|A|}}_{\text{demand: } |A|}, \;\;
    \underbrace{\ell_c, \lambda_c, \phi_c, \; \mathbf{b}_c}_{\text{case: } |A|+3}, \;\;
    \underbrace{\tau_h, \tau_d}_{\text{time: } 2} \,\big],
\end{align*}
with dimension $d = 3|R| + 2|A| + 5$.
...
The reward function guides the agent toward reducing cycle time below a service-level agreement (SLA) threshold $T$, defined as a percentile of the original log's cycle time distribution. Following a two-part design similar to \cite{Learning_policies_drl, Optimizing_Resource_Allocation_Policies}, we combine a directional signal that provides per-step gradient information with a terminal gate that enforces SLA compliance.

\paragraph{Directional signal.}
For a case $\sigma$ with elapsed time $ct(\sigma)$, the directional component is:
\[
r(\sigma) = \frac{K}{ct(\sigma)},
\]
where $K > 0$ is a scaling constant. This signal is inversely proportional to elapsed time: faster cases receive larger rewards, providing a continuous incentive to reduce cycle time at every decision point.

\paragraph{Terminal reward.}
When a case completes, the agent receives a terminal reward that gates on SLA compliance:
\[
R_{\text{terminal}}(\sigma) =
\begin{cases}
K + r(\sigma), & ct(\sigma) < T \\[6pt]
-K, & ct(\sigma) \ge T
\end{cases}
\]
Cases that meet the SLA receive a bonus $K$ on top of the directional signal, reinforcing the full sequence of decisions that led to a fast completion. Cases that violate the SLA receive a flat penalty $-K$, discouraging the agent from allowing any case to exceed the threshold.

\paragraph{Intermediate reward.}
At each decision point during an ongoing case (before completion), the agent receives an intermediate reward based on the elapsed time up to the current simulation time:
\[
R_{\text{intermediate}}(\sigma) =
\begin{cases}
r(\sigma), & ct(\sigma) < T \\[6pt]
-K \cdot \dfrac{ct(\sigma)}{T}, & ct(\sigma) \ge T
\end{cases}
\]
When the case is still within the SLA budget, the agent receives the directional signal $r(\sigma)$, encouraging continued efficiency. When the elapsed time exceeds $T$, the penalty grows linearly with $ct(\sigma)/T$, creating increasing pressure to avoid further delays on cases that have already violated the threshold.

\paragraph{Design rationale.}
The combination of intermediate and terminal rewards addresses the credit assignment problem inherent in long-horizon decision processes. The intermediate reward provides step-level feedback that accelerates learning by signaling the quality of each individual decision without waiting for the case to complete. The terminal reward ensures that the agent's objective remains aligned with the true business goal of SLA compliance. The asymmetric structure—bonus for success, flat or proportional penalty for failure—encourages risk-averse behavior near the threshold boundary. In PPO, these per-step rewards are incorporated into the advantage estimates via generalized advantage estimation (GAE), allowing the value function to learn from denser reward signals and improving both convergence speed and policy quality.
...
\paragraph{Learned state representations.}
The current hand-crafted state vector provides a compact and interpretable input for the agent. A natural extension is to replace it with a learned representation. Given that the simulation state can be serialized as a structured JSON---including resource utilization, active cases, and recent events---a transformer-based encoder could be trained jointly with the DRL agent to produce a dense embedding of the process state. This approach draws on evidence that embedding representations generalize beyond their original domain \cite{Text_Conditional_Image_Generation_with_CLIP}, and on emerging results showing that language models can capture business process semantics \cite{PM_LLM_Benchmark, business_process_modeling_llms}. Such an encoder could allow the agent to exploit richer patterns in the simulation state than a fixed feature vector can express, and would serve as a stepping stone toward zero-shot generalization across processes and objectives.
