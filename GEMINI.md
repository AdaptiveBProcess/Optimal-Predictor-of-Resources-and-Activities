...
*   **Data-Driven Initialization:** The `Initializer` automatically configures simulation parameters (e.g., arrival rates, activity durations, routing probabilities) from real-world event logs (CSV format), facilitating easy setup from existing data. See [Initializer Responsibilities](#initializer-responsibilities) for more.
